...
async def read_question(
    figure_id: int = Path(..., gt=0), id: int = Path(..., gt=0)
...
async def read_all_questions(
    figure_id: int = Path(..., gt=0)
) -> List[QuestionOutputSchema]:
    """
...
async def create_question(
    payload: QuestionInputSchema, figure_id: int = Path(..., gt=0)
) -> QuestionOutputSchema:
    """
    HTTP POST method to create a question
    """
    return await api.create_entity(
        Question, {**payload.__dict__, **{"figure_id": figure_id}}
    )
...
async def update_question(
    ...
    """
    return await api.update_entity(
        Question, {**payload.__dict__, **{"figure_id": figure_id}}, id
    )
